...
The artifact format is documented in [Output Artifacts](/docs/compiler/output-artifacts).
...
The test runner uses vitest. Additional vitest flags can be passed through your `vitest.config.ts` or `package.json` scripts. See [The Test Runner](/docs/testing-and-debugging/test-runner) for details on the testing framework.
...
See [Debugging Compiled Script](/docs/testing-and-debugging/debugging-script) for full usage details.
...
| `-l, --lang <lang>` | `ts` | Target language: `ts`, `go`, `rust`, `python`, `ruby` |
