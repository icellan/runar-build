...
> **Experimental:** The Move frontend compiles contracts correctly but cannot be tested natively with Move tooling. All tests must use the TypeScript test runner (`runar test` with vitest). The API could change in minor releases. For production contracts, consider TypeScript, Go, Rust, or Python.
...
```bash
runar compile src/contracts/counter.runar.move --output ./artifacts
```
...
```bash
runar compile src/contracts/*.runar.move --output ./artifacts
```
...
const source = readFileSync('./src/contracts/Counter.runar.move', 'utf8');
