...
- The `runar` Python package (installed automatically when using the Runar CLI with Python contracts)
...
```bash
runar compile src/contracts/counter.runar.py --output ./artifacts
```
...
```bash
runar compile src/contracts/*.runar.py --output ./artifacts
```
