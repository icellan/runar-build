...
> **Experimental:** The Solidity frontend compiles contracts correctly but cannot be tested natively with Solidity tooling. All tests must use the TypeScript test runner (`runar test` with vitest). The API could change in minor releases. For production contracts, consider TypeScript, Go, Rust, or Python.
...
```bash
runar compile src/contracts/P2PKH.runar.sol --output ./artifacts
```
...
```bash
runar compile src/contracts/*.runar.sol --output ./artifacts
```
...
const source = readFileSync('./src/contracts/Counter.runar.sol', 'utf8');
