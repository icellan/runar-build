...
class FungibleToken extends StatefulSmartContract {
  ...
  public split(
    ...
    const output1 = buildTokenOutput(
      this.getScriptCode(), remainingAmount, this.ownerPubKey
    );
    const output2 = buildTokenOutput(
      this.getScriptCode(), splitAmount, newOwner
    );
...
class MergeableToken extends StatefulSmartContract {
  ...
  public merge(
    ...
    const output = buildTokenOutput(
      this.getScriptCode(), mergedAmount, this.ownerPubKey
    );
...
const wallet = new TokenWallet({
  tokenId: 'abc123...', // Minting transaction ID
  provider: new WhatsOnChainProvider('mainnet'),
...
await wallet.transfer(recipientPubKey, 50n);
...
// Merge only UTXOs below a threshold
await wallet.merge({ minAmount: 100n });
```
...
```typescript
// Safe transfer with automatic retry on UTXO conflict
const result = await wallet.transfer(recipient, amount, {
  retries: 3,
