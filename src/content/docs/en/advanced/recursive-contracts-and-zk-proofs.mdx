...
**Multi-party state.** Multiple parties can drive transitions by requiring different signatures for different methods. The TicTacToe contract in the [examples](/docs/examples/tic-tac-toe) demonstrates this pattern: Alice and Bob take turns, with the contract tracking whose turn it is as state.
