...
Each covenant in the chain enforces that the next transaction maintains specific invariants. The `StatefulSmartContract` pattern in Runar automates this chaining -- see [Recursive Contracts](/docs/advanced/recursive-contracts-and-zk-proofs) for details.
