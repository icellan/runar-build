...
assert(checkSig(sig, pubKey) && verifyRabinSig(msg, rabinSig, oracleKey));
```
...
const oracleValid = verifyRabinSig(msg, rabinSig, oracleKey);
...
| Cross-compiler conformance passes (if multi-language) | `runar test --conformance` |
