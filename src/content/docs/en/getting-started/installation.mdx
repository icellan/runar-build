...
- **Zig 0.14+** -- Only required if using the native Zig compiler. Install from [ziglang.org](https://ziglang.org/download/).
- **Ruby 3.0+** -- Only required if using the native Ruby compiler. Install from [ruby-lang.org](https://www.ruby-lang.org/en/downloads/) or use a version manager like `rbenv`.
...
- **Zig contracts** (`.runar.zig`) -- Use any editor with Zig support; [ZLS](https://github.com/zigtools/zls) (the Zig Language Server) provides autocomplete and diagnostics.
- **Ruby contracts** (`.runar.rb`) -- Use [Ruby LSP](https://github.com/Shopify/ruby-lsp) with the Runar addon for full language support.
