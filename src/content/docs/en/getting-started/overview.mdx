...
Rúnar is a smart contract compiler and SDK for Bitcoin SV. It lets you write contracts in familiar programming languages -- TypeScript, Go, Rust, Python, Zig, Ruby, Solidity, or Move -- and compiles them down to Bitcoin Script for deployment on the BSV network.
...
Rúnar supports eight language frontends. Each compiles through the same intermediate representation (IR) to produce identical Bitcoin Script output:
...
| Zig        | `.runar.zig`   | `runar-zig`      |
| Ruby       | `.runar.rb`    | `runar-rb`       |
| Solidity   | `.runar.sol`   | *(built into compiler)* |
| Move       | `.runar.move`  | *(built into compiler)* |

TypeScript is the primary and most mature frontend. Go, Rust, and Python are stable and conformance-tested. The Zig, Ruby, Solidity, and Move frontends are experimental -- they compile contracts correctly but cannot be tested natively in their respective languages (tests must use the TypeScript test runner).
...
- **Conformance-verified.** The six compiler implementations (TypeScript, Go, Rust, Python, Zig, Ruby) are tested against a shared conformance suite. A contract compiled with `runar-go` produces the same script as the same contract compiled with `runar-rs`.
