...
**Cause:** Public contract methods (`public` methods) are spending conditions. They either succeed (all asserts pass) or fail. They do not return values. Only the `@method` decorator on non-public helper methods can return values, and those values must be on-chain types.
...
**Solution:** Check the inputs you are passing to the contract method. Use `--verbose` test output or the script debugger to inspect the stack at the point of failure. Common causes include incorrect signatures, wrong public keys, or state values that do not match expectations.
...
```bash
runar deploy ./artifacts/MyContract.json --network testnet --dry-run
```

The `--dry-run` flag prints the estimated cost without broadcasting.

### Invalid Network

```
Error: Invalid network 'mainnet'. Artifact was compiled for 'testnet'.
```

**Cause:** The compiled artifact includes a network binding. Deploying to a different network than the one specified at compile time is rejected as a safety measure.

**Solution:** Recompile for the target network:

```bash
runar compile contracts/MyContract.runar.ts --network mainnet --output ./artifacts
```
...
```bash
runar verify <txid> --network testnet --status-only
```
...
```typescript
const contract = await RunarContract.fromTxId(latestTxId, provider);
```
...
```
Error: runar-compiler not found. Ensure @aspect-build/runar-compiler is installed.
```
...
```
Error: Invalid runar.config.ts: unknown option 'target'.
```

**Cause:** The configuration file contains an unrecognized option. This often happens after upgrading Runar to a new version where options have been renamed.

**Solution:** Check the [CLI Reference](/docs/en/api-reference/cli-reference) for the current set of valid configuration options.
...
|-----------|-----------|-------------|
| Compilation error | Read the compiler message and fix the source | `runar compile --verbose` |
| Assert failure | Inspect which assert failed using source map | `runar test --verbose` or `runar debug` |
| Stack overflow | Simplify contract logic | Script debugger with `stack` command |
| Deployment failure | Check wallet balance and network config | `runar deploy --dry-run` |
| Transaction rejection | Verify artifact matches on-chain | `runar verify` |
| Configuration issue | Check versions and config file | `runar --version`, check `runar.config.ts` |
