...
Test behavior can be configured in your `runar.config.ts` or `vitest.config.ts`:
...
export default {
  test: {
    ...
    // Glob pattern for test file discovery (default: "**/*.test.ts")
    include: '**/*.test.ts',

    // Timeout per test in milliseconds (default: 30000)
    timeout: 30000,

    // Enable script execution traces in output (default: false)
    trace: false,

    // Maximum stack depth for script execution (default: 800)
    maxStackDepth: 800,

    // Enable property-based fuzzing tests (default: false)
    fuzz: false,

    // Number of fuzz iterations when fuzzing is enabled (default: 1000)
    fuzzRuns: 1000,
  },
};
```
...
You can also configure the test runner through environment variables:

```bash
# Override test timeout
RUNAR_TEST_TIMEOUT=60000 runar test

# Enable trace output without config change
RUNAR_TEST_TRACE=1 runar test

# Set the random seed for deterministic fuzz runs
RUNAR_FUZZ_SEED=42 runar test
```
...
const result = vm.execute(
  '4830450221...', // unlocking script (hex)
  '76a914...88ac'  // locking script (hex)
);

expect(result.success).toBe(true);
expect(result.stack).toEqual(['01']); // OP_TRUE left on stack
```

The `ScriptVM` does **not** mock cryptographic operations -- it uses real signature verification. Use it when you need to test with actual key pairs and signatures.
...
```bash
# Run the cross-compiler conformance suite
runar test --conformance

# Update golden files after intentional compiler changes
runar test --conformance --update-golden
```

The golden files live in `tests/golden/` and contain the expected SHA-256 hash for each contract/compiler combination. A mismatch indicates a compiler regression or an intentional change that requires updating the golden files.
