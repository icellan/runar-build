...
| Go              | tree-sitter            | 01-parse.ts     |
| Rust            | SWC                    | 01-parse.ts     |
| Python          | Recursive descent      | 01-parse.ts     |
...
Stack IR respects a maximum stack depth of 800 elements. When values are buried deeper than the stack can efficiently reach, the alt-stack is used as overflow storage.

## Optimization Passes

The compiler includes three optional optimization passes that run between the core pipeline stages:

**Peephole optimizer** -- Operates on Stack IR. Applies 15 pattern-matching rules that recognize common opcode sequences and replace them with shorter equivalents. For example, `OP_SWAP OP_DROP` is replaced with `OP_NIP`, and `OP_0 OP_PICK` is replaced with `OP_DUP`.
...
**Constant folder** -- Operates on ANF IR. Evaluates constant expressions at compile time. This optimizer is disabled by default because constant folding can change the script size, which affects fee calculations and may be surprising in audit contexts. Enable it explicitly when you want smaller scripts.
...
| Implementation | Language   | Primary Use Case                          |
| -------------- | ---------- | ----------------------------------------- |
| runar-compiler | TypeScript | Reference implementation, CLI, SDK        |
| runar-go       | Go         | High-performance server-side compilation  |
| runar-rs       | Rust       | Embedded and WASM compilation             |
| runar-py       | Python     | Research, prototyping, Jupyter notebooks  |
