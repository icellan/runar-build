...
The parser reads source code and produces a language-neutral abstract syntax tree. The parser used depends on the source language -- ts-morph for TypeScript, hand-written recursive descent parsers for Solidity, Move, Go, Rust, and Zig, and hand-written tokenizers with recursive descent for Python and Ruby -- but the output is always the same `ContractNode` structure.
...
**Why ANF matters.** The ANF representation serves as the **conformance boundary** between the six independent compiler implementations. All six compilers (TypeScript, Go, Rust, Python, Zig, Ruby) must produce byte-identical ANF for the same source contract. The temporaries are numbered sequentially (`t0`, `t1`, `t2`, ...) following a deterministic left-to-right, depth-first traversal of the AST.
