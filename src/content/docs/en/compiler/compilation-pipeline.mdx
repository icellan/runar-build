...
The parser reads source code and produces a language-neutral abstract syntax tree. The parser used depends on the source language -- ts-morph for TypeScript, tree-sitter for Go, SWC for Rust, and a recursive descent parser for Python -- but the output is always the same `ContractNode` structure.
      ...
      "body": [
        {
          ...
          "expression": {
            ...
            "left": {
              "type": "CallExpression",
              "callee": "hash160",
              "args": [{ "type": "Identifier", "name": "pubKey" }]
            },
            "right": {
...
- The base class (`SmartContract` vs `StatefulSmartContract`) is recorded so downstream passes know whether to inject statefulness machinery.
- Constructor parameters and state fields are separated so the compiler knows which values are baked into the locking script.

You can stop the pipeline here using the `parseOnly` option or the `runar compile --parse-only` CLI flag.
...
You can stop the pipeline here using the `validateOnly` option.
...
You can stop the pipeline here using the `typecheckOnly` option.
...
**Stack depth management.** The stack lowering pass tracks the current stack depth and enforces a maximum of 800 elements. When a value is buried too deep to reach efficiently with `OP_PICK` or `OP_ROLL`, the pass moves it to the alt-stack using `OP_TOALTSTACK` and retrieves it later with `OP_FROMALTSTACK`.
...
Runs after stack lowering, before emission. Applies 15 pattern-matching rules:

| Pattern | Replacement | Savings |
| ------- | ----------- | ------- |
| `OP_SWAP OP_DROP` | `OP_NIP` | 1 byte |
| `OP_SWAP OP_OVER` | `OP_TUCK` | 1 byte |
...
Disabled by default. When enabled, evaluates constant expressions at compile time. For example, `sha256(0x68656c6c6f)` with a literal argument is replaced with its precomputed result. This reduces script size but changes the script hash, which can be surprising during audits.
...
- [Configuration](/docs/en/compiler/configuration) -- Compiler options and CLI flags for controlling the pipeline
- [Output Artifacts](/docs/en/compiler/output-artifacts) -- Detailed format of the compiled artifact JSON
