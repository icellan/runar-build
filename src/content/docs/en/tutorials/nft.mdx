...
- Completed the [Hello World Tutorial](/docs/en/tutorials/hello-world) and the [Fungible Token Tutorial](/docs/en/tutorials/fungible-token)
...
describe('NFTExample', () => {
  ...
  it('should transfer ownership to a new owner', async () => {
    const contract = await TestContract.fromSource(
      'contracts/NFT.runar.ts',
      'NFTExample'
    );

    const instance = contract.create(tokenId, metadata, alicePub);

    const result = await instance.call('transfer', {
  ...
  it('should allow owner to burn the token', async () => {
    ...
    const result = await instance.call('burn', {
      sig: instance.sign(aliceKey),
    });
  ...
  it('should preserve tokenId and metadata across transfers', async () => {
    const contract = await TestContract.fromSource(
      'contracts/NFT.runar.ts',
      'NFTExample'
    );
...
const metadataHash = bsv.crypto.Hash.sha256(
  Buffer.from(JSON.stringify(fullMetadata))
).toString('hex');
```
...
- [Multi-Party Escrow Tutorial](/docs/en/tutorials/multi-party-escrow) -- Learn multi-signature patterns with three-party escrow
- [Auction Example](/docs/en/examples/auction) -- See a stateful contract with competitive bidding and deadlines
- [Example Gallery](/docs/en/examples/gallery) -- Browse all annotated contract examples
