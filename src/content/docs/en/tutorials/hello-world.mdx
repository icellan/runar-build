...
The `contracts/` directory is where your contract source lives. The `tests/` directory is pre-configured with vitest. The `artifacts/` directory will be populated when you compile.

## Writing the Hello World Contract

Create a new file at `contracts/P2PKH.runar.ts`. This implements Pay-to-Public-Key-Hash, the standard Bitcoin pattern where funds can only be spent by proving ownership of the private key that corresponds to a given public key hash.
...
describe('P2PKH', () => {
  it('should unlock with correct key pair', async () => {
    const contract = await TestContract.fromSource(
      'contracts/P2PKH.runar.ts',
      'P2PKH'
    );
    ...
    // Create a contract instance with the public key hash
    const instance = contract.create(pubKeyHash);

    // Unlock should succeed with matching key pair
    const result = await instance.call('unlock', {
      sig: instance.sign(privateKey),
  ...
  it('should fail with wrong key pair', async () => {
    ...
    const correctKey = bsv.PrivateKey.fromRandom('testnet');
    const wrongKey = bsv.PrivateKey.fromRandom('testnet');
    const pubKeyHash = bsv.crypto.Hash.sha256ripemd160(
      correctKey.toPublicKey().toBuffer()
    ).toString('hex');
    ...
    const result = await instance.call('unlock', {
      sig: instance.sign(wrongKey),
      pubKey: wrongKey.toPublicKey().toString(),
    });

    expect(result.success).toBe(false);
  });

  it('should fail when pubKey does not match hash', async () => {
    const contract = await TestContract.fromSource(
      'contracts/P2PKH.runar.ts',
      'P2PKH'
    );

    const keyA = bsv.PrivateKey.fromRandom('testnet');
    const keyB = bsv.PrivateKey.fromRandom('testnet');

    // Lock to keyA's hash
    const pubKeyHash = bsv.crypto.Hash.sha256ripemd160(
      keyA.toPublicKey().toBuffer()
    ).toString('hex');

    const instance = contract.create(pubKeyHash);

    // Sign correctly with keyB, but provide keyB's pubKey (wrong hash)
    const result = await instance.call('unlock', {
      sig: instance.sign(keyB),
      pubKey: keyB.toPublicKey().toString(),
    });

    // Signature is valid for keyB, but hash160(keyB) !== pubKeyHash
    expect(result.success).toBe(false);
  });
});
```
