...
- Runar CLI installed (see [Installation](/docs/getting-started/installation))
...
All Runar contracts import from `runar-lang`. This is the only allowed import in contract files. The import provides the base class (`SmartContract`), assertion function (`assert`), cryptographic types (`PubKey`, `Sig`, `Addr`), and built-in functions (`hash160`, `checkSig`).
...
`Addr` is a 20-byte type representing a BSV address (the RIPEMD-160 hash of a public key). The `readonly` keyword is required for `SmartContract` properties -- stateless contracts cannot have mutable state. This value is embedded directly in the compiled Bitcoin Script.
...
| `assert()` | The assertion primitive -- used to define spending conditions in public methods |
...
- [Fungible Token Tutorial](/docs/tutorials/fungible-token) -- Build a stateful token contract with transfer, send, and merge operations
- [Contract Basics](/docs/writing-contracts/contract-basics) -- Deep dive into the type system, built-in functions, and compiler constraints
