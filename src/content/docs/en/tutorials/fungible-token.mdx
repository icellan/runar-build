...
- Runar CLI installed and a project initialized (see [Hello World Tutorial](/docs/en/tutorials/hello-world))
...
Create `contracts/FungibleToken.runar.ts`:
...
describe('FungibleTokenExample', () => {
  ...
  it('should transfer tokens to a new owner', async () => {
    const contract = await TestContract.fromSource(
      'contracts/FungibleToken.runar.ts',
      'FungibleTokenExample'
    );

    // Alice starts with 100 tokens
    const instance = contract.create(alicePub, 100n);
  ...
  it('should reject transfer exceeding balance', async () => {
    ...
    const instance = contract.create(alicePub, 50n);
  ...
  it('should reject transfer with wrong signature', async () => {
    const contract = await TestContract.fromSource(
      'contracts/FungibleToken.runar.ts',
      'FungibleTokenExample'
    );
  ...
  it('should allow owner to send entire balance', async () => {
    const contract = await TestContract.fromSource(
      'contracts/FungibleToken.runar.ts',
      'FungibleTokenExample'
    );
    ...
    const result = await instance.call('send', {
      sig: instance.sign(aliceKey),
      txPreimage: instance.getPreimage(),
    });
...
| `buildStateOutput` | Serializes all outputs defined by `addOutput` calls |
| UTXO token model | Each balance is a separate UTXO; transfers create new UTXOs |

## What's Next

- [NFT Tutorial](/docs/en/tutorials/nft) -- Build a non-fungible token with unique metadata and transfer/burn operations
- [Multi-Party Escrow Tutorial](/docs/en/tutorials/multi-party-escrow) -- Learn multi-signature patterns with buyer, seller, and arbiter roles
- [Example Gallery](/docs/en/examples/gallery) -- Browse annotated contract examples
