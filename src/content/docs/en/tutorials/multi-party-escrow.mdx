...
```bash
runar compile src/contracts/Escrow.runar.ts --output ./artifacts --asm
```
...
When both parties are satisfied, use the SDK to construct and broadcast a release transaction:
...
const sellerSigner = new LocalSigner('<seller-WIF>');
const buyerSigner = new LocalSigner('<buyer-WIF>');

const contract = await RunarContract.fromTxId(artifact, '<txid>', 0, provider);
contract.connect(provider, sellerSigner);

const result = await contract.call('release', [null, null], {
  // Both signatures are auto-generated from their respective signers
});
```
...
```typescript
const buyerSigner = new LocalSigner('<buyer-WIF>');
const arbiterSigner = new LocalSigner('<arbiter-WIF>');

const contract = await RunarContract.fromTxId(artifact, '<txid>', 0, provider);
contract.connect(provider, buyerSigner);

const result = await contract.call('refund', [null, null], {
  // Both signatures are auto-generated from their respective signers
});
```

See [Calling a Contract](/docs/sdk/calling) and [Multi-Signer Transactions](/docs/sdk/multi-signer) for the full multi-signer workflow.
...
- [Tic-Tac-Toe Example](/docs/examples/tic-tac-toe) -- A two-player on-chain game with state management
