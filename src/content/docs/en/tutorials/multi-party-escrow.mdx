...
- Completed the [Hello World Tutorial](/docs/en/tutorials/hello-world)
...
Create `contracts/Escrow.runar.ts`:
...
describe('Escrow', () => {
  ...
  const sellerKey = bsv.PrivateKey.fromRandom('testnet');
  const arbiterKey = bsv.PrivateKey.fromRandom('testnet');
  const buyerPub = buyerKey.toPublicKey().toString();
  const sellerPub = sellerKey.toPublicKey().toString();
  const arbiterPub = arbiterKey.toPublicKey().toString();

  async function createEscrow() {
    const contract = await TestContract.fromSource(
      'contracts/Escrow.runar.ts',
      'Escrow'
    );
    return contract.create(buyerPub, sellerPub, arbiterPub);
  }

  describe('release', () => {
    it('should succeed with seller + buyer signatures', async () => {
      const instance = await createEscrow();

      const result = await instance.call('release', {
        sellerSig: instance.sign(sellerKey),
        buyerSig: instance.sign(buyerKey),
      });

      expect(result.success).toBe(true);
    });

    it('should fail with wrong seller key', async () => {
      const instance = await createEscrow();
      const wrongKey = bsv.PrivateKey.fromRandom('testnet');

      const result = await instance.call('release', {
        sellerSig: instance.sign(wrongKey),
        buyerSig: instance.sign(buyerKey),
      });
    ...
    it('should fail with wrong buyer key', async () => {
      const instance = await createEscrow();
      const wrongKey = bsv.PrivateKey.fromRandom('testnet');

      const result = await instance.call('release', {
        sellerSig: instance.sign(sellerKey),
        buyerSig: instance.sign(wrongKey),
      });

      expect(result.success).toBe(false);
    });

    it('should fail with only seller signature', async () => {
      const instance = await createEscrow();

      // Seller tries to release alone (using seller key for both)
      const result = await instance.call('release', {
        sellerSig: instance.sign(sellerKey),
        buyerSig: instance.sign(sellerKey),
      });
  ...
  describe('refund', () => {
    it('should succeed with buyer + arbiter signatures', async () => {
      const instance = await createEscrow();

      const result = await instance.call('refund', {
        buyerSig: instance.sign(buyerKey),
        arbiterSig: instance.sign(arbiterKey),
      });

      expect(result.success).toBe(true);
    });

    it('should fail without arbiter agreement', async () => {
      const instance = await createEscrow();

      // Buyer tries to refund alone
      const result = await instance.call('refund', {
        buyerSig: instance.sign(buyerKey),
        arbiterSig: instance.sign(buyerKey),
      });

      expect(result.success).toBe(false);
    });

    it('should fail if seller tries to block refund', async () => {
      const instance = await createEscrow();

      // Seller cannot participate in the refund flow
      const result = await instance.call('refund', {
        buyerSig: instance.sign(buyerKey),
        arbiterSig: instance.sign(sellerKey),
      });
...
- [Example Gallery](/docs/en/examples/gallery) -- Browse annotated contract examples including auctions, games, and oracle-based contracts
- [Counter Example](/docs/en/examples/counter) -- The simplest stateful contract
- [Tic-Tac-Toe Example](/docs/en/examples/token-swap) -- A two-player on-chain game with state management
- [Contract Basics](/docs/en/writing-contracts/contract-basics) -- Deep dive into the type system and compiler constraints
