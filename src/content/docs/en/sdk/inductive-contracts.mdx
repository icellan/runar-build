---
title: "Inductive (Stateful) Contracts"
...
Inductive contracts maintain state across multiple transactions by enforcing that each spend creates a new UTXO with an updated state. This pattern turns a chain of UTXOs into an on-chain state machine.

## What Makes a Contract Inductive

A standard (stateless) contract is a one-shot lock: once its UTXO is spent, the contract is gone. An inductive (stateful) contract enforces that the spending transaction creates a **continuation output** -- a new UTXO locked by the same contract script but with updated state.
...
In Runar, inductive contracts extend `StatefulSmartContract` instead of `SmartContract`:
...
| Type | Encoding |
| ---- | -------- |
| `bigint` | Minimal Script number encoding (variable length) + 1-byte length prefix |
| `boolean` | `OP_TRUE` (0x01) or `OP_FALSE` (0x00) |
| `ByteString` | Raw bytes + varint length prefix |
...
// stateBytes: "0105" (1-byte length prefix + value 5)
...
```typescript
import { StatefulSmartContract, assert, SigHashPreimage } from 'runar-lang';
...
const deployResult = await counter.deploy({ satoshis: 10000 });
...
await counter.call('increment', { preimage: 'auto' }, { outputs: 'auto' });
console.log('After 1st call:', counter.state); // { count: 1n }
console.log('UTXO:', counter.txid, counter.outputIndex);
...
const output = await provider.getOutput(txid, 0);
const state = extractStateFromScript(
  output.script,
  counterArtifact.stateFields,
);
...
const result = await counter.call('increment', {
  ...
  outputs: 'auto',
  fundingUtxos: [additionalUtxo], // extra input to cover fees
});
```
...
- [Token Contracts](/docs/en/sdk/token-contracts) -- Tokens are built on the inductive contract pattern
- [Calling a Contract](/docs/en/sdk/calling) -- General contract calling guide
- [Fee and Change Handling](/docs/en/sdk/fee-and-change-handling) -- Managing fees in state transition chains
