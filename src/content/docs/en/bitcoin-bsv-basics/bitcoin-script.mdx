...
`OP_CHECKSIG` is the cornerstone of authorization on Bitcoin. It verifies that a transaction input was signed by the holder of a specific private key. The OP_PUSH_TX technique (covered in [How Smart Contracts Work on BSV](/docs/bitcoin-bsv-basics/how-smart-contracts-work-on-bsv)) uses `OP_CHECKSIG` in a novel way to enable transaction introspection.
...
Now that you understand how Bitcoin Script works, the next page covers [Transactions and Outputs](/docs/bitcoin-bsv-basics/transactions-and-outputs) --- the data structures that carry scripts on-chain and form the backbone of every contract interaction.
